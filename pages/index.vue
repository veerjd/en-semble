<template>
    ...
    <div v-else-if="potentialMatches.length" class="space-y-6">
      <div v-for="match in potentialMatches" :key="match.id" 
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          <span v-for="interest in match.interests" :key="interest.id"
                class="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">
            {{ interest.name }}
          </span>
        </div>
        <button @click="requestMatch(match.id)" 
  ...
  <div v-else>
    <h1 class="text-3xl font-bold text-center mb-6">Welcome to Interest Matcher</h1>
    <p class="text-center text-gray-600 mb-8">
      Connect with people who share your interests for meaningful conversations.
    </p>
...
const loadPotentialMatches = async () => {
  try {
    ...
    potentialMatches.value = matches.map(match => ({
      ...match,
      interests: match.user_interests.map(ui => ui.interests)
    })).filter(match => 
      match.interests.some(interest => interestIds.includes(interest.id))
    )
  } catch (error) {
